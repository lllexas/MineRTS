...
## Framework Advantages And New TUI Template

### 1. 现有框架的优势

当前这套框架最重要的优势，是层次终于顺了：

- `Query / Execute` 负责资源访问协议
- `ConsoleClientRuntime` 负责前端仲裁
- `Session` 负责生命周期与业务组织
- `TUISelectSlot` 负责交互输入和选择骨架

这意味着以后写新功能时，不需要再让一个巨型类同时承担：
...
这些原本已经在 `TUISelectSlot` 中写好的能力，可以继续作为交互内核存在，不需要每个新界面再重写一遍输入系统。

### 2. 新的 TUI 模板写法

以后新的 console 交互界面，优先采用这样的写法：

- 如果本质是“正文 + 选项”或“列表 + 详情 + 选择确认”
- 优先写成 `XXXSession : TUISelectSlot`

其中职责应当明确分开：

- `Session`
  - 负责生命周期
  - 负责与 `ConsoleManager` 的关系
  - 负责后端业务回调
...
除非这个界面的交互模型已经明显超出 `TUISelectSlot` 的能力边界。
...
- 以后新的资源驱动界面，可以稳定写成：
  - `Query -> VFSQueryResult -> ClientRuntime -> Session / Slot`

也就是说，以后更应该复用这条统一链路，而不是再制造新的“神秘桥接层”。

一句话总结：

这套框架的价值不是“做了更多抽象”，而是以后写新业务界面时，只需要写业务组织和版面，不再重写输入系统。
...
这些东西本来就不应该是互斥方案，而应该是可自由组装的积木。

更准确地说：

- `TUISelectSlot` 负责交互内核
- `TSSStyle + TUITool` 负责排版与视觉结构
- `Session` 负责把不同的 TUI 片段组织成完整交互界面

因此以后新的 TUI 设计，不应该在“正文盒子”与“选项列表”之间二选一，而应该优先思考：

- 这个界面的正文区域是否需要 box
- 这个界面的交互区域是否需要 selection list
- 两者应该怎样在一个 session 中组合

`.msg` 现在采用的就是这种组合模型：

- 上半段是消息正文 box
- 下半段是选项选择区
- 两者共同运行在同一个 `TUISelectSlot` 系 session 中

这条原则应当推广到以后更多资源驱动界面上，而不是继续让每个新界面重新手搓一整套显示与输入系统。
