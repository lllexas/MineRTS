...
/// <summary>
/// 非泛型 GraphView 接口 - 用于窗口解耦喵~
/// </summary>
public interface INekoGraphView
{
    BasePackData SerializeToPackGeneric();
    void PopulateFromPackGeneric(BasePackData pack);
    NodeSystem CurrentSystem { get; }
...
/// 仅编辑器使用喵~
/// </summary>
public abstract class BaseGraphView<TPack> : GraphView, INekoGraphNodeFactory, INekoGraphView
    where TPack : BasePackData
{
    /// <summary>
    /// 当前系统类型喵~
    /// </summary>
    public abstract NodeSystem CurrentSystem { get; }
    ...
    /// 实现接口：通用序列化喵~
    /// </summary>
    public BasePackData SerializeToPackGeneric() => SerializeToPack();

    /// <summary>
    /// 实现接口：通用反序列化喵~
    /// </summary>
    public void PopulateFromPackGeneric(BasePackData pack)
    {
        if (pack is TPack tPack)
        {
            PopulateFromPack(tPack);
        }
        else
        {
            Debug.LogError($"[NekoGraph] 类型不匹配喵！期望 {typeof(TPack).Name}，实际收到 {pack?.GetType().Name} 喵~");
        }
    }
    /// <summary>
    ...
    /// <summary>
    /// 设置 PackID 喵~（从窗口工具栏同步过来）
    /// </summary>
    public virtual void SetPackID(string packID)
    {
        CurrentPackID = packID ?? string.Empty;
    }

    /// <summary>
    /// 获取当前 PackID 喵~
    /// </summary>
    public virtual string GetPackID()
    {
    ...
    /// <summary>
    /// 序列化后处理喵~（子类可重写，用于填充特殊字段）
    /// </summary>
    protected virtual void OnAfterSerialize(TPack pack)
    {
        // 默认实现：不做任何处理
        // 子类可以重写此方法来填充 Pack 的特殊字段（如 RootNodeIds 等）
    }
