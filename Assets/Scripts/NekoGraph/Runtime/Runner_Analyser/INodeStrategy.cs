...
/// 
/// 设计理念：
/// 1. 只有阻隔信号的节点才需要保存状态（如 Spine、Trigger）
/// 2. 是否阻隔由策略决定，不是节点数据类的责任
/// 3. 策略知道节点的所有状态细节，负责捕获和恢复
/// 
/// 实现此接口的策略会被 GraphRunner 在存档时调用喵~
/// </summary>
public interface IBlockingNodeStrategy
{
    ...
    /// <returns>状态对象（会被 JSON 序列化）</returns>
    ...
    /// </summary>
    /// <param name="node">节点数据</param>
    /// <param name="state">状态对象（从 JSON 反序列化）</param>
