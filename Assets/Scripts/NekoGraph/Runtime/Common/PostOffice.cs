...
/// 所有的 GameEvent 都必须在这里“挂号、检查、投递”，严禁私自绕过邮局走底层 PostSystem 喵~
/// </summary>
public static class PostOffice
{
    /// <summary>
    /// 【标准投递窗口】
    /// 自动根据 GameEvent 关联的协议进行校验喵！
    /// </summary>
    /// <param name="evt">事件枚举</param>
    /// <param name="payload">符合协议的参数对象</param>
    public static void Send(GameEvent evt, object payload)
    {
        var meta = TriggerRegistry.GetMeta(evt);
        if (meta == null)
        {
            Debug.LogError($"[PostOffice] 试图发送未定义的事件: {evt}！请先在 GameEvent 枚举中注册喵~");
            return;
        }
    ...
    /// </summary>
    public static void Send(GameEvent evt) => Send(evt, null);
